...
func TestWork_BlocksPattern(t *testing.T) {
	tempDir := t.TempDir()
	...
	if !strings.Contains(filename, "gocamo_000_test_blocks_") {
...
func TestWork_MixedPatternTypes(t *testing.T) {
	...
	// Blocks pattern job
	blocksCfg := &config.Config{
		Width: 20, Height: 20, BasePixelSize: 4, PatternType: "blocks",
	}
	jobs <- Job{
		Camo:       config.CamoColors{Name: "blocks", Colors: []string{"ff0000", "00ff00"}},
		Index:      0,
		Config:     blocksCfg,
		OutputPath: outputDir,
	}
...
// Benchmark tests
func BenchmarkWork_BlocksPattern(b *testing.B) {
	tempDir := b.TempDir()
...
// TestWork_SharedConfigConcurrentJobs guards against the data race where
// concurrent jobs sharing one Config pointer raced on cfg.ColorRatios
// (GeneratePattern now mutates a per-job copy). Run with -race to verify.
func TestWork_SharedConfigConcurrentJobs(t *testing.T) {
	tempDir := t.TempDir()
	...
	for i := range jobCount {
		jobs <- Job{
			Camo:       config.CamoColors{Name: "shared", Colors: []string{"ff0000", "00ff00", "0000ff"}},
			Index:      i,
			Config:     cfg,
			OutputPath: tempDir,
		}
