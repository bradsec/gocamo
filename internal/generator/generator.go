...
func GeneratePattern(ctx context.Context, cfg *config.Config, camo config.CamoColors, index int, outputPath string) error {
	...
	// Jobs share the same Config pointer across worker goroutines and
	// SetColorRatios writes cfg.ColorRatios, so mutate a per-job copy.
	localCfg := *cfg
	if err := localCfg.SetColorRatios(len(colors)); err != nil {
	...
	case "woodland":
		gen = &WoodlandGenerator{}
	case "multicam":
		gen = &MulticamGenerator{}
	case "blocks":
		gen = &BlocksGenerator{}
	case "blob":
		gen = &BlobGenerator{}
	...
	fileName := fmt.Sprintf("gocamo_%03d_%s_%s_%s_w%dx%d.png",
		index, sanitizeName(camo.Name), colorCodesStr, patternName, cfg.Width, cfg.Height)
	filePath := filepath.Join(outputPath, fileName)

	return saveImageToFile(img, filePath)
}

// unsafeNameChars matches every character not allowed in generated filenames.
var unsafeNameChars = regexp.MustCompile(`[^a-zA-Z0-9_-]+`)

// sanitizeName makes a palette name safe for use in an output filename.
// Palette names come from user-supplied JSON and must not be able to inject
// path separators or other special characters into the output path.
