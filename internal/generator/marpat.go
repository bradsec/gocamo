...
// MarpatGenerator creates marpat digital camouflage patterns.
...
// Generate creates a marpat digital camouflage pattern.
// Pipeline: Voronoi seeding → directional CA for rectangular shaping → pixel blocks → ratio enforcement.
func (pg *MarpatGenerator) Generate(ctx context.Context, cfg *config.Config, colors []color.RGBA) (image.Image, error) {
	...
	// Use marpat default ratios when the user has not specified explicit ratios.
	ratios := cfg.ColorRatios
	if cfg.RatiosString == "" {
		ratios = pg.getMarpatDefaultColorRatios(len(colors))
	}
	...
	// Places proper-sized rectangular patches so corrections match the marpat style.
...
func (pg *MarpatGenerator) generateVoronoiGrid(width, height int, ratios []float64, numColors int) [][]int {
	grid := make([][]int, height)
	for i := range grid {
		grid[i] = make([]int, width)
	}

	// Keep seed spacing in grid cells so BasePixelSize scales marpat the same way
...
func (pg *MarpatGenerator) applyRectangularCA(grid [][]int, width, height, winW, winH, iterations int) {
	halfW := winW / 2
	halfH := winH / 2
...
func (pg *MarpatGenerator) applyDigitalPixelBlocks(grid [][]int, width, height int) {
...
func (pg *MarpatGenerator) renderGrid(img *image.NRGBA, grid [][]int, colors []color.RGBA, pixelSize int) {
	bounds := img.Bounds()
...
func (pg *MarpatGenerator) initializeMarpatGrid(width, height int, colorRatios []float64, numColors int) [][]int {
	grid := make([][]int, height)
	for i := range grid {
		grid[i] = make([]int, width)
	}
	...
	if len(ratios) == 0 || len(ratios) != numColors {
		ratios = pg.getMarpatDefaultColorRatios(numColors)
	}
...
func (pg *MarpatGenerator) enforceColorRatios(grid [][]int, width, height int, ratios []float64, numColors int) {
	total := width * height
	...
	for iter := 0; iter < 10; iter++ {
		...
		// Patch size (3–7 wide, 2–5 tall grid cells) mirrors the marpat cluster scale.
...
func (pg *MarpatGenerator) countColors(grid [][]int, width, height, numColors int) []int {
...
// getMarpatDefaultColorRatios returns marpat's default colour distribution ratios.
func (pg *MarpatGenerator) getMarpatDefaultColorRatios(numColors int) []float64 {
	if numColors == 4 {
