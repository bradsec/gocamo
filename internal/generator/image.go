...
func (ig *ImageGenerator) Generate(ctx context.Context, cfg *config.Config, _ []color.RGBA) (image.Image, []color.RGBA, error) {
	...
	mainColors, err := kMeansClustering(ctx, pixels, cfg.KValue, 100)
	if err != nil {
		return nil, nil, fmt.Errorf("error extracting colors from image: %w", err)
	}
...
func kMeansClustering(ctx context.Context, pixels []color.Color, k int, maxIterations int) ([]color.RGBA, error) {
	if k <= 0 {
		return nil, fmt.Errorf("k-means requires at least one cluster, got k=%d", k)
	}

	if len(pixels) == 0 {
		return nil, fmt.Errorf("k-means requires at least one pixel")
	}
...
func resizeAndCropImage(img image.Image, targetWidth, targetHeight int) image.Image {
	...
	// A zero-area source would make scaleFactor infinite below.
