...
func TestGeneratePattern_ValidBlocks(t *testing.T) {
	cfg := &config.Config{
		Width:         100,
		Height:        100,
		BasePixelSize: 4,
	...
	if !strings.HasSuffix(filename, "_blocks_w100x100.png") {
		t.Errorf("Filename should end with '_blocks_w100x100.png', got: %s", filename)
	}
...
func TestGeneratePattern_ValidBlob(t *testing.T) {
	cfg := &config.Config{
	...
	// Check filename format
	filename := files[0].Name()
	if !strings.HasPrefix(filename, "gocamo_005_blob_test_") {
		t.Errorf("Filename should start with 'gocamo_005_blob_test_', got: %s", filename)
	}
	if !strings.Contains(filename, "_blob_") {
		t.Errorf("Filename should contain '_blob_', got: %s", filename)
	}
}
