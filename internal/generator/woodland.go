...
// WoodlandGenerator creates woodland (military-inspired) camouflage patterns.
// Mimics proven military camouflage design principles rather than novel algorithms.
type WoodlandGenerator struct{}
...
func (pg *WoodlandGenerator) Generate(ctx context.Context, cfg *config.Config, colors []color.RGBA) (image.Image, error) {
	...
	// Apply woodland-style layered approach with weighted color distribution
	...
	// Layer 3: Small digital elements (woodland-style pixels)
...
func (pg *WoodlandGenerator) findBackgroundColor(colors []color.RGBA) int {
	maxBrightness := 0
	backgroundIndex := 0
...
func (pg *WoodlandGenerator) addLargeShapes(grid [][]int, cellWidth, cellHeight int, colors []color.RGBA, backgroundIndex int, cfg *config.Config) {
...
func (pg *WoodlandGenerator) addWoodlandElements(grid [][]int, cellWidth, cellHeight int, colors []color.RGBA, cfg *config.Config) {
...
// addDigitalDetails creates woodland-style small digital elements with weighted colors
func (pg *WoodlandGenerator) addDigitalDetails(grid [][]int, cellWidth, cellHeight int, colors []color.RGBA, cfg *config.Config) {
	...
	for i := 0; i < numPixels; i++ {
		...
		// woodland-style rectangular digital elements
...
func (pg *WoodlandGenerator) applyMilitarySmoothing(grid [][]int, cellWidth, cellHeight, numColors int) {
...
func (pg *WoodlandGenerator) drawOrganicShape(grid [][]int, centerX, centerY, sizeX, sizeY, colorIndex, cellWidth, cellHeight int) {
...
func (pg *WoodlandGenerator) drawVerticalElement(grid [][]int, startX, startY, colorIndex, cellWidth, cellHeight int) {
...
func (pg *WoodlandGenerator) drawHorizontalElement(grid [][]int, startX, startY, colorIndex, cellWidth, cellHeight int) {
...
func (pg *WoodlandGenerator) drawClusterElement(grid [][]int, centerX, centerY, colorIndex, cellWidth, cellHeight int) {
