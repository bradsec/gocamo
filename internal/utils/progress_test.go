...
func TestTrackProgress_LargeNumbers(t *testing.T) {
	...
	// Drain the pipe concurrently: 1000 progress bar prints exceed the
	// pipe buffer, so reading only after completion deadlocks the writer.
...
func TestPrintProgressBar_WidthValidation(t *testing.T) {
	...
	// Should handle all widths without crashing. Bars are carriage-return
	// separated (each print rewrites the same console line).
	var bars []string
	for _, segment := range strings.Split(output, "\r") {
		if strings.TrimSpace(segment) != "" {
			bars = append(bars, segment)
		}
	}
	if len(bars) != len(widths) {
		t.Errorf("Expected %d progress bars, got %d", len(widths), len(bars))
	}

	// Each bar should be well formed
	for i, bar := range bars {
		if !strings.Contains(bar, "[") || !strings.Contains(bar, "]") {
			t.Errorf("Progress bar %d should contain brackets: %s", i, bar)
		}
	}
}
