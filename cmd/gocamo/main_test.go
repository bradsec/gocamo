...
func TestRun_ValidBlocksPatternWithColors(t *testing.T) {
	outputDir := t.TempDir()
	cfg := &config.Config{
		Width:         100,
		Height:        100,
	...
	err := run(cfg)
	if err != nil {
		t.Errorf("unexpected error for valid blocks pattern: %v", err)
	}
...
func TestRun_ValidBlobPatternWithJSON(t *testing.T) {
	outputDir := t.TempDir()
	...
	err = run(cfg)
	if err != nil {
		t.Errorf("unexpected error for valid blob pattern: %v", err)
	}
